...
from host_guest.io import filetyper, coords_library, hdf5_library
from host_guest.energy import docker


def complexes_from_folder(host_folder, monomer_folder, number_of_host, number_of_monomers, number_of_complexes, energy, results_folder, seed=None, hdf5=False):
    """
    ...
    hdf5: bool, optional
        If True, write the docked complexes to 'complexes.h5' (host atoms,
        lattice and labels stored once per base_name, guest coordinates per
        complex/load) instead of 'complexes.json'.
    """
    seen = []
    ...
    # ---- Edit: 13 June 2026, by Adriana Ugarte ----
    # Skip the JSON molecule file entirely when writing HDF5 output.
    if hdf5:
        hdf5_filename = os.path.join(results_folder, 'complexes.h5')
    else:
        ...
        for monomer_file in monomer_folder:
            ...
            if base_name not in seen:
                energy_dict, complex_molecules = docker.Dock(
                    host_system, monomer, number_of_host, number_of_monomers, number_of_complexes, energy, seed
                )
                # ---- Edit: 13 June 2026, by Adriana Ugarte ----
                if hdf5:
                    hdf5_library.append_hdf5_atom(
                        base_name, complex_molecules, host_system, monomer, number_of_monomers, hdf5_filename)
                else:
...
def complexes_from_file(host_system_file, monomer_file, number_of_host, number_of_monomers, number_of_complexes, energy,  results_folder, seed=None, hdf5=False):
    """
    ...
    monomer_base_name = os.path.basename(monomer_file).split('.')[0]
    base_name = host_base_name + '_' + monomer_base_name
    seed = seed  # Ensure seed is passed correctly
...
def complex_from_ase_atom(list_of_hosts, list_of_monomers, number_of_host, number_of_monomers, number_of_complexes, energy, selector, results_folder, seed=None, hdf5=False):
    """
...
def main():
    ...
    # Edit: Added seed argument for reproducibility
    parser.add_argument('-s', '--seed', type=int, default=None,
                        help='Random seed for reproducibility (default is None)')

    # Edit: 13 June 2026, by Adriana Ugarte
    parser.add_argument('--hdf5', action='store_true',
                        help='Write complexes.h5 (HDF5) instead of complexes.json')
    ...
    complexes_from_folder(host_folder, monomer_folder, args.number_of_host,
                                         args.number_of_monomers, args.number_of_complexes, args.energy,  args.results_folder, args.seed, args.hdf5)  # Edit: Added seed and hdf5 arguments
...
def main2():
    ...
    complexes_from_file(args.host_file, args.monomer_file, args.number_of_host, args.number_of_monomers, args.number_of_complexes, args.energy, args.results_folder, args.seed, args.hdf5)  # Edit: Added seed and hdf5 arguments
...
def main3():
    ...
    complex_from_ase_atom(args.host_file, args.monomer_file, args.number_of_host,
                                       args.number_of_monomers, args.number_of_complexes, args.energy, args.selector,  args.results_folder, args.seed, args.hdf5)  # Edit: Added seed and hdf5 arguments
